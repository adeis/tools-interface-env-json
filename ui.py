...
<body>
            ...
            <div id="workspace-selected" style="display: none; height: 100%; flex-direction: column;">
                ...
                <div class="pane active" id="pane-form" style="margin-top: 1.5rem;">
                    <div class="form-grid">
                        ...
                        <div class="form-group form-full-width">
                            <label class="form-label" for="domain_api">Domain API</label>
                            <input type="text" class="form-control" id="domain_api" placeholder="e.g. https://company.app.nusawork.com">
                            <span class="form-helper-text">Base URL of the company API. Modifying this updates the Token & Attendance API URLs if they are not customized.</span>
                        </div>
                    ...
                    <div class="helper-box" id="autocomplete-box" style="display: none;">
                        ...
                        <div>
                            ...
                            <p>We've automatically suggested or synchronized API URLs based on your inputs.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const inputDomainApi = document.getElementById('domain_api');
        ...
        // Helper to extract the base domain (origin) from URLs
        function getDomainFromUrls(tokenUrl, attendanceUrl) {
            const url = tokenUrl || attendanceUrl || '';
            if (!url) return '';
            try {
                const urlObj = new URL(url);
                return urlObj.origin;
            } catch(e) {
                // simple split fallback instead of regex with slashes
                if (url.includes('://')) {
        ...
        function selectCompany(id) {
            ...
            const domain = getDomainFromUrls(data.token_api_url, data.attendance_api_url);
            inputDomainApi.value = domain;
            
            inputTokenUrl.value = data.token_api_url || '';
            inputAttendanceUrl.value = data.attendance_api_url || '';
            
            // Set autogenerated flags based on whether they match expected values or are empty
            const expectedTokenUrl = domain ? `${domain}/auth/api/oauth/token` : '';
            const expectedAttendanceUrl = domain ? `${domain}/api/attendance-raw/client/storage-data` : '';
            
            inputTokenUrl.dataset.autogenerated = (!data.token_api_url || data.token_api_url === expectedTokenUrl) ? 'true' : 'false';
            inputAttendanceUrl.dataset.autogenerated = (!data.attendance_api_url || data.attendance_api_url === expectedAttendanceUrl) ? 'true' : 'false';
        ...
        // Autocomplete from Domain API
        function handleDomainApiChange() {
            let domain = inputDomainApi.value.trim();
            
            // Normalize domain: prepend https:// if it does not start with http:// or https://
            let normalizedDomain = domain;
            if (domain && !/^https?:\/\//i.test(domain)) {
                normalizedDomain = 'https://' + domain;
            }
            
            if (inputTokenUrl.dataset.autogenerated === 'true') {
                inputTokenUrl.value = normalizedDomain ? `${normalizedDomain}/auth/api/oauth/token` : '';
            }
            if (inputAttendanceUrl.dataset.autogenerated === 'true') {
                inputAttendanceUrl.value = normalizedDomain ? `${normalizedDomain}/api/attendance-raw/client/storage-data` : '';
            }
            
            if (domain && (inputTokenUrl.dataset.autogenerated === 'true' || inputAttendanceUrl.dataset.autogenerated === 'true')) {
        ...
        // Autocomplete Attendance URL when Token URL changes
        function handleTokenUrlChange() {
            inputTokenUrl.dataset.autogenerated = 'false';
            
            const tokenUrl = inputTokenUrl.value.trim();
            const attendanceUrl = inputAttendanceUrl.value.trim();
            
            // Keep Domain API input in sync
            const parsedDomain = getDomainFromUrls(tokenUrl, attendanceUrl);
            inputDomainApi.value = parsedDomain;
            ...
            if (tokenUrl && (!attendanceUrl || inputAttendanceUrl.dataset.autogenerated === 'true')) {
                try {
                    ...
                    if (tokenUrl.includes('://')) {
                        ...
                        const domainPart = parts[0] + '//' + parts[2];
                        inputAttendanceUrl.value = `${domainPart}/api/attendance-raw/client/storage-data`;
                        inputAttendanceUrl.dataset.autogenerated = 'true';
                        autocompleteBox.style.display = 'flex';
                    }
                }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            inputDomainApi.oninput = handleDomainApiChange;
            ...
            inputAttendanceUrl.oninput = () => {
                ...
                // Sync Domain API if token url is empty
                if (!inputTokenUrl.value.trim()) {
                    inputDomainApi.value = getDomainFromUrls('', inputAttendanceUrl.value.trim());
                }
                autocompleteBox.style.opacity = '0';
                setTimeout(() => { autocompleteBox.style.display = 'none'; }, 200);
            };
